...
const Map = React.forwardRef((props, ref) => {
    ...
    useEffect(() => {
        Logger.setLogCallback((log) => {
            ...
            if (message.match(/Requesting.+failed.+MGLNativeNetworkManager/)
                || message.match(/offline/)
                || message.match(/You're calling `setMaxZoom` after the `MapView` was destroyed, were you invoking it after `onDestroy()`?/)) {
