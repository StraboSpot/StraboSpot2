...
const AddThreeDStructureModal = ({onPress}) => {
  ...
  const pageKey = PAGE_KEYS.THREE_D_STRUCTURES;
  const subpages = Object.values(THREE_D_STRUCTURE_TYPES);
  ...
  const renderForm = (formProps) => {
    ...
    // full QEM form
      ...
      return (
        <>
          {subpages[selectedTypeIndex] === THREE_D_STRUCTURE_TYPES.FAULT && <AddFault {...subformProps}/>}
          {subpages[selectedTypeIndex] === THREE_D_STRUCTURE_TYPES.FOLD && <AddFold {...subformProps}/>}
          {subpages[selectedTypeIndex] === THREE_D_STRUCTURE_TYPES.OTHER && <AddOther {...subformProps}/>}
          {subpages[selectedTypeIndex] === THREE_D_STRUCTURE_TYPES.TENSOR && <AddTensor {...subformProps}/>}
        </>
      );
    }
  };
